...
public class GameScreen : BaseScreen
{
        ...
        ListboxDefinition? listBox = null)
    {
        ...
        if (popupBox != null)
        {
            ...
            // Each popup closes itself: capture the dialog (and its own click handler) in
            // the close callback so stacked popups (e.g. several cities completing their
            // production in one turn) can each be dismissed independently. The previous
            // shared-field approach left every popup but the last one unclosable, which
            // froze the game on the leftover popup.
            CivDialog popupDialog = null!;
            popupDialog = new CivDialog(MainWindow, dialog, (a1, a2, a3, a4) =>
            {
                CloseDialog(popupDialog);
                handleButtonClick?.Invoke(a1, a2, a3, a4);
            });
            ShowDialog(popupDialog, stack: true);
        }
    }
