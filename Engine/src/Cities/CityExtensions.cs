...
namespace Civ2engine
{
    public static class CityExtensions
    {
        ...
        /// <summary>
        /// The improvements whose effects apply to this city: its own improvements plus any
        /// civ-wide wonders elsewhere in the civilization (e.g. the Hanging Gardens). Use this
        /// instead of <c>city.Improvements</c> when summing effects so civ-wide wonders count.
        /// </summary>
        public static IEnumerable<Improvement> EffectImprovements(this City city)
        {
            var own = city.Improvements;
            var civCities = city.Owner?.Cities;
            if (civCities == null)
            {
                return own;
            }
            var civWideElsewhere = civCities
                .Where(c => c != city)
                .SelectMany(c => c.Improvements)
                .Where(i => i.CivWide);
            return own.Concat(civWideElsewhere);
        }
        ...
        {
            var storageBuildings = city.EffectImprovements()
