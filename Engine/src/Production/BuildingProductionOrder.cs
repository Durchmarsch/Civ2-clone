...
namespace Civ2engine.Production
{
    public class BuildingProductionOrder : ProductionOrder
    {
        ...
        public override bool CompleteProduction(City city, Rules rules)
        {
            ...
            // Wonders are world-unique: record it and remove it from every civ's production
            // list so no one else can build it, and cities mid-build switch to something else.
            if (Improvement.IsWonder)
            {
                ProductionPossibilities.RegisterWonderBuilt(this, Improvement.Type);
            }
        ...
        {
            // A wonder already built anywhere in the world can never be built again.
            if (Improvement.IsWonder && ProductionPossibilities.WonderAlreadyBuilt(Improvement.Type))
            {
                return false;
            }
