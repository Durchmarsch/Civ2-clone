...
public class MovementFunctionsTests
{
    ...
    #region IsCurrentlyVisible Tests

    [Fact]
    public void IsCurrentlyVisible_FriendlyUnitOnAdjacentTile_ReturnsTrue()
    {
        // Regression: after a unit moves, the fog-of-war reveal relies on
        // IsCurrentlyVisible detecting a friendly unit on a *neighbour* of the tile
        // being evaluated. A copy-paste bug checked the centre tile's units instead of
        // the neighbour's, so newly-revealed tiles were filtered out of the redraw and
        // stayed black on the map.
        // Arrange
        var unitTile = _map.TileC2(5, 5);
        var revealedTile = unitTile.Neighbours().First();
        revealedTile.SetVisible(_civ.Id); // unit has already seen this tile
        var unit = CreateTestUnit(UnitGas.Ground, _civ);
        unit.CurrentLocation = unitTile;
        unitTile.UnitsHere.Add(unit);

        // Act - revealedTile must count as currently visible because a friendly unit
        // sits on an adjacent tile (unitTile)
        var result = _map.IsCurrentlyVisible(revealedTile, _civ.Id);
